...
// Test script for Session Buddy with Sidepanels
console.log('Starting Session Buddy with Sidepanels tests...');
...
describe('Session Buddy with Sidepanels', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });

  test('switchPanel', () => {
    sidepanelUtils.switchPanel('sidepanels/main-sp.html');
    expect(chrome.runtime.sendMessage).toHaveBeenCalledWith(
      { action: 'switchPanel', panelPath: 'sidepanels/main-sp.html' },
      expect.any(Function)
    );
  });

  test('saveCurrentSession', () => {
    sidepanelUtils.saveCurrentSession();
    expect(chrome.runtime.sendMessage).toHaveBeenCalledWith(
      { action: 'saveSession' },
      expect.any(Function)
    );
  ...
  test('createNewSession', () => {
    ...
    sidepanelUtils.createNewSession(sessionName);
    expect(chrome.runtime.sendMessage).toHaveBeenCalledWith(
  ...
  test('restoreSession', () => {
    sidepanelUtils.restoreSession('test-session-id');
    expect(chrome.runtime.sendMessage).toHaveBeenCalledWith(
      { action: 'restoreSession', sessionId: 'test-session-id' },
      expect.any(Function)
    );
  });

  test('deleteSession', () => {
    sidepanelUtils.deleteSession('test-session-id');
    expect(chrome.runtime.sendMessage).toHaveBeenCalledWith(
      { action: 'deleteSession', sessionId: 'test-session-id' },
      expect.any(Function)
    );
  });
