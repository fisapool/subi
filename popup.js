...
function renderSessionList() {
  ...
  const sortedSessions = [...currentSessions].sort((a, b) => (b.createdAt || 0) - (a.createdAt || 0))
  ...
  sortedSessions.forEach((session) => {
    ...
    sessionItem.innerHTML = `
      <div class="session-header p-4 flex items-center justify-between cursor-pointer">
        ...
        <div class="actions">
          <button class="btn btn-primary restore-session" data-session-id="${session.id}">Restore</button>
          <button class="btn btn-secondary delete-session" data-session-id="${session.id}">Delete</button>
        </div>
      </div>
    `
...
async function restoreSession(sessionId) {
  try {
    ...
    // Check for warnings
    if (response.cookieResults && response.cookieResults.warnings && response.cookieResults.warnings.length > 0) {
      console.warn("Session restored with warnings:", response.cookieResults.warnings)

      // Show a non-blocking notification
      updateStatus(`Session restored with ${response.cookieResults.warnings.length} warnings`, false, true)
    } else {
